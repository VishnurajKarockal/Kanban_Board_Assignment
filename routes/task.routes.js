...
TaskRouter.get("/", auth, access("regular user", "admin"), async (req, res) => {
  ...
  try {
    if (status === "to-do") {
      res
        .status(200)
        .json({ tasks: await TaskModel.find({ status: "to-do" }) });
    } else if (status === "in-progress") {
      res
        .status(200)
        .json({ tasks: await TaskModel.find({ status: "in-progress" }) });
    } else if (status === "done") {
      res.status(200).json({ tasks: await TaskModel.find({ status: "done" }) });
    } else {
      if (userID && page) {
        const limit = 10;
